...
class Transform(Dataset):
    """
    A wrapper dataset class that applies image transformations to an existing subset.
    Can handle standard single images or image pairs (for Siamese networks).

    Args:
        subset (Dataset or Subset): The dataset containing (image, label) or (img1, img2, label) tuples.
        flag (bool, default=False): If True, treats subset as containing image pairs for Siamese networks.
        augment (bool, default=True): Whether to apply data augmentation (only if train=True).
        train (bool, default=True): Whether the dataset is used for training.
        img_size (int, default=224): The target image size after resizing.
    """
    def __init__(self, subset, flag=False, augment=True, train=True, img_size=224):
        self.subset = subset
        self.train = train
        ...
        # CIFAR-100 dataset normalization values (mean and std per channel)
        ...
        if self.train and self.augment:
            ...
            self.transform = transforms.Compose([
                ...
                transforms.RandomCrop(img_size, padding=4),   # Random crop with padding
                transforms.RandomHorizontalFlip(),            # Random horizontal flip
                transforms.RandomRotation(10),                # Small rotation
                transforms.ColorJitter(
                    brightness=0.1,                          # Slight color jitter
                    contrast=0.1,
                    saturation=0.1,
                    hue=0.05
                ),
                transforms.ToTensor(),                        # Convert PIL image to tensor
                self.normalize                                # Normalize using CIFAR-100 stats
            ])
        else:
            # Validation or test pipeline (no augmentation)
            self.transform = transforms.Compose([
    ...
    def __getitem__(self, idx):
        """
        Get a transformed sample from the dataset.

        Returns:
            If flag=False: (img_tensor, label)
            If flag=True: (img1_tensor, img2_tensor, label)
        """
        if self.flag:
            # Siamese mode: return transformed image pairs and label
